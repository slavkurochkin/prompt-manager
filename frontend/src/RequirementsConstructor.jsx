...
function RequirementsConstructor({ onNavigateBack, loadedPrompt, onPromptSaved }) {
  ...
  const parsePromptContent = useCallback((content) => {
    ...
    sections.forEach(section => {
      ...
      if (header === '## System Prompts') {
        const contentLines = lines.slice(1).map(l => l.trim()).filter(l => l)
        const validPrompts = []
        const customPrompts = []
        
        contentLines.forEach(line => {
          if (SYSTEM_PROMPTS.includes(line)) {
            validPrompts.push(line)
          } else {
            customPrompts.push(line)
          }
        })
        
        setSystemPrompts(validPrompts)
        setCustomSystemPrompt(customPrompts.join('\n'))
      } else if (header === '## Technical Architecture') {
  ...
  useEffect(() => {
    if (loadedPrompt && loadedPrompt.content) {
      ...
      if (content && lastLoadedPromptId.current !== promptId) {
        ...
        // Expand System Prompts, Technical Architecture and Design Styles sections when loading a prompt so user can see loaded values
  ...
  const autoGeneratedPrompt = useMemo(() => {
    ...
    if (systemPrompts.length > 0 || customSystemPrompt) {
      const systemPromptsList = [...systemPrompts]
      if (customSystemPrompt) systemPromptsList.push(customSystemPrompt)
      sections.push('## System Prompts\n' + systemPromptsList.join('\n'))
    }
  ...
  const mergeAutoGeneratedContent = (existingPrompt, newAutoGenerated, previousAutoGenerated) => {
      if (!newAutoGenerated || newAutoGenerated === 'No requirements specified yet. Fill in the form to generate a prompt.') {
        ...
        // If previousAutoGenerated is empty but existingPrompt has content, 
        // remove all auto-generated items from existingPrompt (user cleared everything)
        if (existingPrompt && existingPrompt.trim() && existingPrompt !== 'No requirements specified yet. Fill in the form to generate a prompt.') {
          // Parse existing prompt and remove all auto-generated items
          const sections = existingPrompt.split(/\n\n(?=## )/g)
          ...
          sections.forEach(section => {
            ...
            if (header && header.startsWith('##')) {
              ...
              // Keep only custom items (not auto-generated)
              ...
              // If no custom items, skip the section entirely
          ...
          return cleanedSections.length > 0 ? cleanedSections.join('\n\n') : 'No requirements specified yet. Fill in the form to generate a prompt.'
        }
      ...
      if (!previousAutoGenerated || previousAutoGenerated === '') {
        ...
        // Separate non-section content (content without ## headers) from sections
        const existingSectionSections = []
        ...
        existingSections.forEach(section => {
          ...
          if (header && header.startsWith('##')) {
            ...
            existingSectionSections.push({ header, section, lines })
          } else if (section.trim()) {
            // Non-section content (content without ## headers) - keep at top
        ...
        existingSectionSections.forEach(({ header, section, lines }) => {
          ...
          if (newSection) {
            ...
            // Also remove items that are no longer in newItems (they were cleared)
            ...
            existingItemMap.forEach((value, key) => {
              // Check if this is an auto-generated item (not a custom item)
              const isAutoGenerated = value.includes('Framework:') || value.includes('Database:') || value.includes('Database Migrations:') ||
                value.includes('Vector Database:') || value.includes('Messaging:') || value.includes('Caching Strategy:') ||
            ...
            // Only add section if there's content (items or custom text)
            if (mergedContent.trim()) {
              mergedSections.push(header + '\n' + mergedContent)
            }
            // If mergedContent is empty, skip this section entirely (all items were cleared)
          } else {
            // Section doesn't exist in new auto-generated - check if it only has auto-generated items
            const existingContent = lines.slice(1).join('\n').trim()
            const existingItems = existingContent ? existingContent.split('\n').map(l => l.trim()).filter(l => l) : []
            
            // Check if all items are auto-generated (no custom items)
            const hasOnlyAutoGenerated = existingItems.length > 0 && existingItems.every(item => {
              const key = item.split(':')[0]?.trim()
              return key && (
                item.includes('Framework:') || item.includes('Database:') || item.includes('Database Migrations:') ||
                item.includes('Vector Database:') || item.includes('Messaging:') || item.includes('Caching Strategy:') ||
            ...
            // If section only has auto-generated items and doesn't exist in new prompt, remove it
            // Otherwise, keep it (it has custom items)
            if (!hasOnlyAutoGenerated) {
              mergedSections.push(section)
            }
            // If hasOnlyAutoGenerated is true, skip this section (all items were cleared)
          }
        })
        ...
        // Combine everything: non-section content first, then sections
        const result = [...nonSectionContent, ...mergedSections].filter(Boolean).join('\n\n')
      ...
      // Separate non-section content from sections
      const existingSectionSections = []
      ...
      existingSectionSections.forEach(({ header, section, lines }) => {
        ...
        if (newSectionMap.has(header)) {
          ...
          // Strategy: Update/add/remove auto-generated items based on new content
          // Preserve custom items that don't match auto-generated patterns
          
          // If we have previousAutoGenerated, we need to detect removed items
          if (previousAutoGenerated && previousAutoGenerated !== '') {
            // Parse previous auto-generated to see what items existed before
            ...
            if (prevSection) {
              ...
              const prevContent = prevLines.slice(1).join('\n').trim()
              const prevItems = prevContent ? prevContent.split('\n').map(l => l.trim()).filter(l => l) : []
              ...
              // Remove items that were in previous but not in new (they were cleared)
          ...
          // If allContent is empty, skip this section entirely (all items were cleared)
        } else {
          // Section doesn't exist in new auto-generated - check if it only has auto-generated items
          const existingContent = lines.slice(1).join('\n').trim()
          const existingItems = existingContent ? existingContent.split('\n').map(line => line.trim()).filter(line => line) : []
      ...
      // Combine: non-section content first, then sections
      return [...nonSectionContent, ...mergedSections].filter(Boolean).join('\n\n')
    }
    ...
    if (autoGeneratedPrompt !== lastAutoGenerated || justClearedRef.current) {
      ...
      if (isEditing) {
        ...
        setEditedPrompt(prev => {
          // Use the current edited prompt as the baseline to preserve all content
          // Pass lastAutoGenerated so we can detect and remove cleared items
          if (prev && prev.trim()) {
            // When we have existing content (loaded or previously merged), merge with it
            // Pass lastAutoGenerated to properly detect removed items
            // If lastAutoGenerated is empty, use the current prev as baseline for comparison
            const previousForComparison = lastAutoGenerated || prev
            return mergeAutoGeneratedContent(prev, autoGeneratedPrompt, previousForComparison)
          } else {
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      ...
      const shouldUpdate = hasLoadedPrompt && loadedPrompt?.id && saveMode === 'update'
      
      const url = shouldUpdate ? `${API_BASE}/${loadedPrompt.id}` : API_BASE
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ 
          ...
          model: shouldUpdate ? (loadedPrompt?.model || null) : null,
          token_count: tokenCount,
          rating: shouldUpdate ? (loadedPrompt?.rating || 0) : 0,
          note: shouldUpdate ? (loadedPrompt?.note || null) : null,
          tags: shouldUpdate ? (loadedPrompt?.tags || ['requirements', 'constructor']) : ['requirements', 'constructor']
        }),
      })

      if (!response.ok) throw new Error(`Failed to ${shouldUpdate ? 'update' : 'save'} prompt`)

      showToast(`Prompt ${shouldUpdate ? 'updated' : 'saved'} successfully!`)
      
      // If creating new, clear title. If updating, keep it.
      if (!shouldUpdate) {
        setPromptTitle('')
      }
      
      // Refresh the prompts list in the parent component
      if (onPromptSaved) {
        onPromptSaved()
      }
    } catch (err) {
      showToast(`Failed to ${hasLoadedPrompt && saveMode === 'update' ? 'update' : 'save'} prompt`, 'error')
    ...
    <div className="requirements-constructor">
      <div className="requirements-layout">
          ...
          <div className="requirements-form">
            ...
            <section className="requirements-section">
              <div 
                ...
                onClick={() => toggleSection('systemPrompts')}
                    ...
                    <div className="multi-select-container">
                      {SYSTEM_PROMPTS.map(item => (
                        <button
                          key={item}
                          type="button"
                          className={`multi-select-chip ${systemPrompts.includes(item) ? 'selected' : ''}`}
                          onClick={() => toggleSystemPrompt(item)}
                        >
                          {item}
                          {systemPrompts.includes(item) && <Check size={12} />}
                        </button>
                      ))}
                    </div>
                    <input
                      type="text"
                      placeholder="Or add custom system prompt..."
        ...
        <div className="requirements-right-panel">
          ...
          {/* Title input and save mode selector */}
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-primary)' }}>
            <input
              ...
              className="form-input"
              style={{ marginBottom: hasLoadedPrompt ? '12px' : '0' }}
            />
            {hasLoadedPrompt && loadedPrompt?.id && (
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Save as:</span>
                <div style={{ display: 'flex', gap: '4px', background: 'var(--bg-secondary)', borderRadius: '6px', padding: '2px' }}>
                  <button
                    type="button"
                    ...
                    style={{
                      ...
                      borderRadius: '4px',
                      border: 'none',
                      background: saveMode === 'update' ? 'var(--accent-primary)' : 'transparent',
                      color: saveMode === 'update' ? 'white' : 'var(--text-primary)',
                      cursor: 'pointer',
                      fontWeight: saveMode === 'update' ? '500' : '400',
                      transition: 'all 0.2s'
                    }}
                  >
                    Update Existing
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: '4px',
                      border: 'none',
                      background: saveMode === 'create' ? 'var(--accent-primary)' : 'transparent',
                      color: saveMode === 'create' ? 'white' : 'var(--text-primary)',
                      cursor: 'pointer',
                      fontWeight: saveMode === 'create' ? '500' : '400',
                      transition: 'all 0.2s'
                    }}
                  >
                    Create New
